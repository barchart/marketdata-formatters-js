module.exports = (() => {
	...
	const symbolParser = {
		parseInstrumentType: (symbol) => {
			...
			const longFutureOptionMatch = symbol.match(longFutureOptionRegex);
			...
			if (futureOptionMatch !== null) {
				const month = futureOptionMatch[2];
		...
		getProducerSymbol: (symbol) => {
			...
			if (instrumentType !== null && instrumentType.type === 'future') {
				...
				const currentDate = new Date();
				const currentYear = currentDate.getFullYear();
				const optionType = instrumentType.option_type === 'call' ? 'C' : 'P';
				const optionTypeTrans = String.fromCharCode(optionType.charCodeAt(0) + (instrumentType.year - currentYear));

				if (instrumentType.root.length < 3) {
					return instrumentType.root + instrumentType.month + instrumentType.strike + optionTypeTrans;
				} else {
					const year = instrumentType.year.toString().substr(-1);
		...
		getFuturesOptionPipelineFormat: (symbol) => {
			const instrument = symbolParser.parseInstrumentType(symbol);

			if (instrument === null || instrument.type !== 'future_option') {
				return null;
			}

			const optionType = instrument.option_type === 'call' ? 'C' : 'P';
